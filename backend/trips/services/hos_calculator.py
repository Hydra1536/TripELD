...
def calculate_hos(
    ...
    avg_speed_mph = (total_distance_miles / total_trip_hours) if (total_trip_hours and total_distance_miles) else 55.0
    ...
    while len(daily_logs) < 60:
        ...
        for block in FIXED_DAILY_SCHEDULE:
            ...
            if typ == "DRIVING":
                ...
                if remaining_trip_hours > 1e-9:
                    ...
                    allowed_drive = min(block_len, remaining_trip_hours, remaining_cycle_hours, MAX_DAILY_DRIVING_HOURS)
                    # add driving portion
                    start_actual = start
                    ...
                    if remainder > 1e-9:
                        ...
                        if remaining_trip_hours <= 1e-9 and remaining_cycle_hours > 0:
                            ...
                            remaining_cycle_hours = max(0.0, remaining_cycle_hours - remainder)
                        else:
                    ...
                    if fuel_stops and (not fuel_stop_used) and allowed_drive > 0 and remaining_cycle_hours > 0:
                        ...
                        remaining_cycle_hours = max(0.0, remaining_cycle_hours - FUEL_STOP_TIME)
                        fuel_stop_used = True
                else:
        ...
        off_duty_hours = sum(a["duration"] for a in activities if a["type"] == "OFF_DUTY")
        sleeper_hours = sum(a["duration"] for a in activities if a["type"] == "SLEEPER_BERTH")
        total_hours = round(driving_hours + on_duty_hours + off_duty_hours + sleeper_hours, 4)

        miles_today = round(driving_hours * avg_speed_mph, 2) if avg_speed_mph > 0 else 0.0
